...
'''
Usage:
$ spark-submit  --driver-memory=4g --executor-memory=4g --conf "spark.blacklist.enabled=false" stringindexer.py hdfs:/user/bm106/pub/MSD/cf_train_new.parquet hdfs:/user/bm106/pub/MSD/cf_validation.parquet hdfs:/user/bm106/pub/MSD/cf_test.parquet
'''
...
def main(spark, train_path, val_path, test_path):
    ...
    indexer_all.write().overwrite().save('hdfs:/user/te2049/indexer_all.parquet')


    train_idx = indexer_all.transform(train)
    train_idx.repartition(5000,'user_idx')
    train_idx.write.mode('overwrite').parquet('hdfs:/user/te2049/train_index_all.parquet')
